...
//FUNCTION UKURAN PEMUSATAN DATA
function meanFunction(nilaiData) {
	...
	html += '<li><b>Langkah 2:</b><br> Hitung mean dengan cara total nilai data dibagi dengan total data, sehingga didapati hasil demikian <b>'+totalNilaiData+' / '+totalData+' = '+mean.toFixed(3)+'</b></li>';
	return html;
}
...
function meanKelompokFunction(pembilang, penyebut) {
	...
	html = '<li><b>Langkah 1:</b><br> rumus mean data berkelompok dicari dengan persamaan <br>'+
  	...
  	'Dari data di atas, kalikan titik tengah baris ke-x dengan frekuensi baris ke-x, kemudian jumlahkan semua hasil perkalian sehingga didapatkan hasil <b>'+atas+'</b></li>';
  	html += '<li><b>Langkah 2:</b><br> jumlahkan semua frekuensi yang ada pada data tersebut, hasilnya adalah <b>'+bawah+'</b></li>';
	html += '<li><b>Langkah 3:</b><br> Hasil langkah 1 dibagi langkah 2, hasilnya adalah <b>'+atas+' / '+bawah+' = '+meanTabel.toFixed(3)+'</b></li>';
	return html;
}
...
function medianKelompokFunction(arrayFkk, arrayTB) {
	var html = "";
	var dataMedian = (0.5*arrayFkk[arrayFkk.length-1]).toFixed(3);
	//ambil index baris data median
	var index = 0;
	for(var i=0; i<arrayFkk.length; i++) {
		if(parseFloat(dataMedian) <= parseFloat(arrayFkk[i].toFixed(3))) {
			index = i;
			break;
		}
	}
	...
	if(Number.isInteger(arrayTB[index])) {
		...
		batasBawahMedian = arrayTB[index]; //Kontinu tetap
	}

	//hitung Median
	var median = batasBawahMedian+(((parseFloat(dataMedian)-fkkSebelum)/arrayFkk[arrayFkk.length-1])*arrayFkk.length);
	...
	' = '+(0.5*arrayFkk[arrayFkk.length-1]).toFixed(3)+'</b></li>';
	...
	html += '<li><b>Langkah 4:</b><br> masukan semua nilai ke dalam rumus,<b> Me = '+batasBawahMedian+' + (('+dataMedian+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+arrayFkk.length+' = '+median.toFixed(3)+'</b></li>';
	return html;
}
...
function modusKelompokFunction(arrayFrek, arrayTB) {
	...
	{
		batasBawahModus = arrayTB[index]; //Kontinu tetap
	}

	//hitung modus
	var modus = batasBawahModus + ((d1/(d1+d2))*arrayFrek.length);
	...
	' / ('+d1+' + '+d2+')) * '+arrayFrek.length+' = '+modus.toFixed(3)+'</b></li>';
	return html;
}
...
function kuartilFunction(nilaiData, nomorKuartil) {
	...
	')) = '+kuartil.toFixed(3)+'</b></li>';

	return html;
}
function kuartilKelompokFunction(nomorKuartil, arrayFkk, arrayTB) {
	var html = "";
	var letakKuartil = ((nomorKuartil*arrayFkk[arrayFkk.length-1])/4).toFixed(3);

	var index = 0;
	for(var i=0; i<arrayFkk.length; i++)
	{
		if(parseFloat(letakKuartil) <= parseFloat(arrayFkk[i].toFixed(3)))
		{
			index = i;
			break;
		}
	...
	var kuartil = batasBawahKuartil+(((parseFloat(letakKuartil)-fkkSebelum)/arrayFkk[arrayFkk.length-1])*arrayFkk.length);
	...
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> K('+nomorKuartil+') = '+batasBawahKuartil+' + (('+letakKuartil+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+arrayFkk.length+' = '+kuartil.toFixed(3)+'</b></li>';
	return html;
}
...
function desilKelompokFunction(nomorDesil, arrayFkk, arrayTB) {
	var html = "";
	var letakDesil = ((nomorDesil*arrayFkk[arrayFkk.length-1]) / 10).toFixed(3);

	var index = 0;
	for(var i=0; i<arrayFkk.length; i++)
	{
		if(parseFloat(letakDesil) <= parseFloat(arrayFkk[i].toFixed(3)))
		{
			index = i;
			break;
		}
	...
	var desil = batasBawahDesil+(((parseFloat(letakDesil)-fkkSebelum)/arrayFkk[arrayFkk.length-1])*arrayFkk.length);
	...
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> D('+nomorDesil+') = '+batasBawahDesil+' + (('+letakDesil+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+arrayFkk.length+' = '+desil.toFixed(3)+'</b></li>';
	return html;
}
function persentilFunction(nilaiData, nomorPersentil) {
	...
	nilaiData[index-1]+' + (('+letakPersentil+' - '+lkX+') * ('+nilaiData[(index)]+' - '+nilaiData[index-1]+')) = '+persentil.toFixed(3)+'</b></li>';

	return html;
}
function persentilKelompokFunction(nomorPersentil, arrayFkk, arrayTB) {
	var html = "";
	var letakPersentil = ((nomorPersentil*arrayFkk[arrayFkk.length-1]) / 100).toFixed(3);

	var index = 0;
	for(var i=0; i<arrayFkk.length; i++)
	{
		if(parseFloat(letakPersentil) <= parseFloat(arrayFkk[i].toFixed(3)))
		{
			index = i;
			break;
		}
	...
	var persentil = batasBawahPersentil+(((parseFloat(letakPersentil)-fkkSebelum)/arrayFkk[arrayFkk.length-1])*arrayFkk.length);
	...
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> K('+nomorPersentil+') = '+batasBawahPersentil+' + (('+letakPersentil+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+arrayFkk.length+' = '+persentil.toFixed(3)+'</b></li>';
	return html;
}
...
function varianFunction(nilaiData, cekVarian) {
	...
	for(i = 0; i < nilaiData.length; i++) {
		sum += (parseInt(nilaiData[i]));
	}

	var mean = sum/nilaiData.length;

	for(i = 0; i < nilaiData.length; i++) {
		pembilang += Math.pow((parseInt(nilaiData[i]) - parseInt(mean.toFixed(3))),2); 
	}

	if(cekVarian == "sampel") {
		penyebut = (nilaiData.length) - 1;
		varian = parseInt(pembilang.toFixed(3)) / penyebut;
		...
		sum+' / '+nilaiData.length+' = '+mean.toFixed(3)+' </b></li>';
		...
		html += '<li><b>Langkah 4:</b><br> Jumlahkan semua hasil langkah ketiga, sehinga didapatkan hasil <b>'+
		pembilang.toFixed(3)+'</b></li>';
		html += '<li><b>Langkah 5:</b><br> Bagi hasil pada langkah 4 dengan jumlah data, hasilnya adalah <b>'+
		pembilang.toFixed(3)+' / '+penyebut+' = '+varian.toFixed(3)+'</b></li>';
	}
	return html;
}
...
function varianKelompokFunction(cekVarian) {
	var varianPembilang = Math.pow(meanKFPembilang.toFixed(3), 2);

	if(cekVarian == "sampel") {
		var varianPenyebut = meanKFPenyebut - 1;
		var varianKelompok = varianPembilang * varianKFPembilang / varianPenyebut;
		...
		html += '<li><b>Langkah 3:</b><br> Cari hasil jumlah <b>fi * (Xi)^2 = '+varianKFPembilang+'</b></li>';
		html += '<li><b>Langkah 4:</b><br> Hitung total frekuensi atau jumlah sampel data berkelompok, lalu dikurangi 1 = <b>'+
		meanKFPenyebut+' - 1 = '+varianPenyebut+'</b></li>';
		html += '<li><b>Langkah 5:</b><br> Masukan hasil langkah 2-4 ke dalam persamaan, <b>('+varianPembilang+' * '+varianKFPembilang+') / '+varianPenyebut+' = '+varianKelompok.toFixed(3)+'</b></li>';
	}
	else {
		var varianPenyebut = meanKFPenyebut;
		var varianKelompok = varianPembilang * varianKFPembilang / varianPenyebut;
...
function sdFunction(nilaiData, cekSD) {
	...
	for(i = 0; i < nilaiData.length; i++) {
		pembilang += Math.pow(((nilaiData[i]*1) - mean),2); 
	}

	if(cekSD == "sampel") {
		penyebut = nilaiData.length - 1;
		sd = pembilang.toFixed(3) / penyebut;
		...
		' Hasilnya adalah <b>'+pembilang.toFixed(3)+' / '+penyebut+' = V'+sd.toFixed(3)+' = '+
		(Math.sqrt(sd.toFixed(3),2)).toFixed(3)+'</b>';
	}
	else {
		penyebut = nilaiData.length;
		sd = pembilang.toFixed(3) / penyebut;

		html = '<li><b>Langkah 1:</b><br> Persamaan standar deviasi pengamatan populasi adalah <br>'+
		'<img style="width:40%; padding-left: 30%; padding-top:0px; padding-bottom:0px;" src="img/sd-berkelompok-populasi.png"><br>'+
		'dimana: <br>'+
		...
		html += '<li><b>Langkah 4:</b><br> Jumlahkan semua hasil langkah ketiga, sehinga didapatkan hasil <b>'+
		pembilang.toFixed(3)+'</b>';
		html += '<li><b>Langkah 5:</b><br> Bagi hasil pada langkah 4 dengan jumlah data, kemudian diakarkan.'+
		' Hasilnya adalah <b>'+pembilang.toFixed(3)+' / '+penyebut+' = '+sd.toFixed(3)+' = '+
		(Math.sqrt(sd.toFixed(3),2)).toFixed(3)+'</b>';
	}
	return html;
}
function sdKelompokFunction(cekVarian) {
	var varianPembilang = Math.pow(meanKFPembilang.toFixed(3), 2);

	if(cekVarian == "sampel") {
		var varianPenyebut = meanKFPenyebut - 1;
		var varianKelompok = Math.sqrt(varianPembilang * varianKFPembilang / varianPenyebut);
		...
		html += '<li><b>Langkah 4:</b><br> Hitung total frekuensi atau jumlah sampel data berkelompok, lalu dikurangi 1 = <b>'+
		meanKFPenyebut+' - 1 = '+varianPenyebut+'</b></li>';
		html += '<li><b>Langkah 5:</b><br> Masukan hasil langkah 2-4 ke dalam persamaan, lalu diakarkan. <b>V(('+varianPembilang+' * '+varianKFPembilang+') / '+varianPenyebut+') = '+varianKelompok.toFixed(3)+'</b></li>';
	}
	else {
		var varianPenyebut = meanKFPenyebut;
		var varianKelompok = Math.sqrt(varianPembilang * varianKFPembilang / varianPenyebut);

		html = '<li><b>Langkah 1:</b><br> Persamaan standar deviasi data berkelompok pengamatan populasi adalah <br>'+
		'<img style="width:40%; padding-left: 30%; padding-top:0px; padding-bottom:0px;" src="img/sd-berkelompok-populasi.png"><br>'+
		'dimana: <br>'+
		'σ= standar deviasi populasi<br>'+
		'Xi = nilai data ke-i<br>'+
		...
		html += '<li><b>Langkah 4:</b><br> Hitung total frekuensi atau jumlah sampel data berkelompok, N = <b>'+
		varianPenyebut+'</b></li>';
		html += '<li><b>Langkah 5:</b><br> Masukan hasil langkah 2-4 ke dalam persamaan, lalu diakarkan. V<b>(('+varianPembilang+' * '+varianKFPembilang+') / '+varianPenyebut+') = '+varianKelompok.toFixed(3)+'</b></li>';
	}
	return html;
}
...
function medianKelompokFunction(arrayFkk, arrayTB) {
	...
	html += '<li><b>Langkah 3:</b><br> Pada tabel, lihat kolom frekuensi kumulatif untuk melihat median termasuk dalam frekuensi kumulatif yang mana. Lalu ambil nilai frekuensi kumulatif sebelum kelas median, fk dari tabel di atas adalah <b>'+fkkSebelum+'</b></li>';
	html += '<li><b>Langkah 3:</b><br> Tepi bawah kelas median didapat dari batas bawah dikurangi 0.5 jika data diskrit. Jika data kontinu tepi bawahnya sama dengan batas bawah. Tepi bawah median adalah <b>'+batasBawahMedian+'</b></li>';
	html += '<li><b>Langkah 4:</b><br> masukan semua nilai ke dalam rumus,<b> Me = '+batasBawahMedian+' + (('+dataMedian+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+interval+' = '+median.toFixed(3)+'</b></li>';
	return html;
}
function modusFunction(nilaiData) {
	var html = "";
...
function modusKelompokFunction(arrayFrek, arrayTB) {
	...
	html += '<li><b>Langkah 4:</b><br> Hitung d2 kelas modus, <b> d1 = '+arrayFrek[index]+' - '+frekSetelah+' = '+d2+'</b></li>';
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> Mo = '+batasBawahModus+' + ('+d1+
	' / ('+d1+' + '+d2+')) * '+interval+' = '+modus.toFixed(3)+'</b></li>';
	return html;
}
//FUNCTION UKURAN LOKASI
function kuartilFunction(nilaiData, nomorKuartil) {
...
function kuartilKelompokFunction(nomorKuartil, arrayFkk, arrayTB) {
	...
	var kuartil = batasBawahKuartil+(((parseFloat(letakKuartil)-fkkSebelum)/frekuensiKuartil)*interval);
	...
	'Tb = tepi bawah kelas kuartil<br>'+
  	...
  	html += '<li><b>Langkah 3:</b><br> Pada tabel, lihat kolom frekuensi kumulatif untuk melihat letak kuartil termasuk dalam frekuensi kumulatif yang mana. Lalu ambil nilai frekuensi kumulatif sebelum kelas kuartil, fk dari tabel di atas adalah <b>'+fkkSebelum+'</b></li>';
	html += '<li><b>Langkah 4:</b><br> Tepi bawah kelas kuartil didapat dari batas bawah dikurangi 0.5 jika data diskrit. Jika data kontinu tepi bawahnya sama dengan batas bawah. Tepi bawah kuartil adalah <b>'+batasBawahKuartil+'</b></li>';
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> K('+nomorKuartil+') = '+batasBawahKuartil+' + (('+letakKuartil+' - '+fkkSebelum+')/ '+frekuensiKuartil+') * '+interval+' = '+kuartil.toFixed(3)+'</b></li>';
	return html;
}
function desilFunction(nilaiData, nomorDesil) {
	var desil = 0;
...
function desilKelompokFunction(nomorDesil, arrayFkk, arrayTB) {
	...
	var desil = batasBawahDesil+(((parseFloat(letakDesil)-fkkSebelum)/frekuensiDesil)*interval);
	...
	'Tb = tepi bawah kelas desil<br>'+
  	...
  	html += '<li><b>Langkah 3:</b><br> Pada tabel, lihat kolom frekuensi kumulatif untuk melihat letak desil termasuk dalam frekuensi kumulatif yang mana. Lalu ambil nilai frekuensi kumulatif sebelum kelas desil, fk dari tabel di atas adalah <b>'+fkkSebelum+'</b></li>';
	html += '<li><b>Langkah 4:</b><br> Tepi bawah kelas desil didapat dari batas bawah dikurangi 0.5 jika data diskrit. Jika data kontinu tepi bawahnya sama dengan batas bawah. Tepi bawah desil adalah <b>'+batasBawahDesil+'</b></li>';
	html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> D('+nomorDesil+') = '+batasBawahDesil+' + (('+letakDesil+' - '+fkkSebelum+')/ '+frekuensiDesil+') * '+interval+' = '+desil.toFixed(3)+'</b></li>';
	return html;
}
function persentilFunction(nilaiData, nomorPersentil){
	var html = "";
	...
	'dx = nilai data ke-x <br>' +
	'Dari data di atas, nilai persentilnya adalah <br><b>' + 
	nilaiData[lkX-1]+' + (('+letakPersentil+' - '+lkX+') * ('+nilaiData[lkX]+' - '+nilaiData[lkX-1]+')) = '+persentil.toFixed(3)+'</b></li>';
...
function varianFrekuensiFunction(arrVariable, arrFrek, cekVarian) {
	...
	if(cekVarian == "sampel") {
		...
		'n = jumlah sampel</li>';
		html += '<li><b>Langkah 2:</b><br> Cari mean dari data di atas, mean-nya adalah <b>'+
		sum+' / '+totalFrek+' = '+mean.toFixed(3)+' </b></li>';
...
function sdFunction(nilaiData, cekSD) {
	...
	if(cekSD == "sampel") {
		...
		html += '<li><b>Langkah 5:</b><br> Kurangi jumlah data dengan 1, hasilnya adalah <b>'+penyebut+'</b>';
		html += '<li><b>Langkah 6:</b><br> Bagi hasil pada langkah 4 dan langkah 5, kemudian diakarkan.'+
		' Hasilnya adalah <b>V('+pembilang.toFixed(3)+' / '+penyebut+') = '+sd.toFixed(3)+'</b>';
	}
	else {
		penyebut = nilaiData.length;
		sd = Math.sqrt(parseFloat(pembilang.toFixed(3)) / penyebut);
...
function sdFrekuensiFunction(arrVariable, arrFrek, cekVarian) {
	...
	if(cekVarian == "sampel") {
		...
		pembilang.toFixed(3)+'</b></li>';
		html += '<li><b>Langkah 5:</b><br> Kurangi jumlah data dengan 1, hasilnya adalah <b>'+penyebut+'</b></li>';
		html += '<li><b>Langkah 6:</b><br> Bagi hasil pada langkah 4 dan langkah 5, kemudian diakarkan. Hasilnya adalah <b>V('+pembilang.toFixed(3)+' / '+penyebut+') = '+sd.toFixed(3)+'</b></li>';
	}
	else {
		penyebut = totalFrek;
		sd = Math.sqrt(parseFloat(pembilang.toFixed(3)) / penyebut);
...
function meanTunggal(arrData, arrFrekuensi){ //FUNCTION BARU**
	var total = 0;
	var frekuensi = 0;
	...
	return hasilMean.toFixed(3);
}
function meanKelompok(arrData, arrFrekuensi){ //FUNCTION BARU**
	var total = 0;
	var frekuensi = 0;
	for(var i=0; i<arrData.length; i++){
...
function meanTanpaFrekuensi(arrData, tipeData){ //FUNCTION BARU**
	var total = 0;
	for(var i=0; i<arrData.length; i++){
...
function htmlMean(arrData, arrFrekuensi, jenisData){
	...
	if(jenisData == "tunggal")
	{
		if(arrFrekuensi.length == 0) //tanpa frekuensi
		{
			...
			html += '<li><b>Langkah 1:</b><br> Jumlahkan semua nilai data, sehingga didapatkan total nilai data <b>'+totalNilaiData+'</b>. Hitung juga total data, sehingga didapatkan total data <b>'+arrData.length+'</b></li>';
 			html += '<li><b>Langkah 2:</b><br> Hitung mean dengan cara total nilai data dibagi dengan total data, sehingga didapati hasil demikian <b>'+totalNilaiData+' / '+arrData.length+' = '+(totalNilaiData/arrData.length).toFixed(3)+'</b></li>';
		}
		else
		{
			var totalNilaiData = 0;
			...
			html += '<li><b>Langkah 1:</b><br> Jumlahkan semua nilai data, sehingga didapatkan total nilai data <b>'+totalNilaiData+'</b>. Hitung juga total data, sehingga didapatkan total data <b>'+arrData.length+'</b></li>';
 			html += '<li><b>Langkah 2:</b><br> Hitung mean dengan cara total nilai data dibagi dengan total data, sehingga didapati hasil demikian <b>'+totalNilaiData+' / '+totalData+' = '+(totalNilaiData/totalData).toFixed(3)+'</b></li>';
		}
	}
	else if(jenisData == "kelompok")
	{
		...
		html += '<li><b>Langkah 1:</b><br> rumus mean data berkelompok dicari dengan persamaan <br>';
 		...
 		html += '<li><b>Langkah 3:</b><br> Hasil langkah 1 dibagi langkah 2, hasilnya adalah <b>'+totalNilaiData+' / '+totalData+' = '+(totalNilaiData/totalData).toFixed(3)+'</b></li>';
	}
	return html;
}
...
function htmlMedianKelompok(arrayData, arrayFrek, arrayFkk) //**
{
	var dataMedian = (0.5*arrayFkk[arrayFkk.length-1]).toFixed(3);
	var index = 0;
	for(var i=0; i<arrayFkk.length; i++) {
	...
	//ambil batas bawah
	var batasBawahMedian = parseFloat(arrayData[i].split('-')[0]);
	...
	var html = '<li><b>Langkah 1:</b><br> rumus median data berkelompok dicari dengan persamaan <br>'+
	 ...
	 ' = '+dataMedian+'</b></li>';
	 html += '<li><b>Langkah 3:</b><br> Pada tabel, lihat kolom frekuensi kumulatif untuk melihat median termasuk dalam frekuensi kumulatif yang mana. Lalu ambil nilai frekuensi kumulatif sebelum kelas median, fk dari tabel di atas adalah <b>'+fkkSebelum+'</b></li>';
	 html += '<li><b>Langkah 4:</b><br> Tepi bawah kelas median didapat dari batas bawah dikurangi 0.5 jika data diskrit. Jika data kontinu tepi bawahnya sama dengan batas bawah. Tepi bawah median adalah <b>'+batasBawahMedian+'</b></li>';
	 html += '<li><b>Langkah 5:</b><br> masukan semua nilai ke dalam rumus,<b> Me = '+batasBawahMedian+' + (('+dataMedian+' - '+fkkSebelum+')/ '+arrayFkk[arrayFkk.length-1]+') * '+arrayFkk.length+' = '+median.toFixed(3)+'</b></li>';
	 return html;
}
...
function medianKelompok(arrayData, arrayFrek, arrayFkk, jenisVariabel) { //**
	...
	{
		batasAtasMedian = parseFloat(arrayData[i].split('-')[1])+1;
	}
	else if(jenisVariabel == 'kontinu')
	{
		batasAtasMedian = parseFloat(arrayData[i].split('-')[1]);
	}
	var interval = (batasAtasMedian - batasBawahMedian).toFixed(1);
...
function htmlMedian(arrData, arrFrekuensi, jenisData){ //**
	var html = "";
	if(jenisData == "tunggal")
	{
		if(arrFrekuensi.length == 0) //tanpa frekuensi
		{
			...
			var temp = document.getElementById(arrData);
			var joinMedian = arrData.join(",");
			...
			html = '<li><b>Langkah 1:</b><br> Urutkan data dari nilai terkecil ke nilai terbesar\n'+
			joinMedian+'</li>';
...
function modusKelompok(arrData, arrFrekuensi, jenisVariabel) //**
{
	...
	var interval = (batasAtas - batasBawah).toFixed(1);
...
function randomSoalDataULTunggalDiskrit(nilaiMin, nilaiMax, jumlahData){ //FUNCTION BARU**
	var strData = "";
	var min=nilaiMin;
	var max=nilaiMax;
...
function buatSoalUKP() 
{
	...
	var arrOpsi = randomOpsi(10,10,jumlahLulus,"diskrit");
	...
	html = htmlMedianKelompok(arrData, arrFrekuensi, arrFkk);
	...
	//Kuartil: Soal 1
	randomData = randomSoalDataKelompokDiskrit(20,6,100);
	frekuensi = randomSoalDataTunggalDiskrit(0,1, 100);
	arrData = randomData.split(',');
	arrFrekuensi = frekuensi.split(',');
	shuffle(arrFrekuensi);
	hasilModus = modusKelompok(arrData, arrFrekuensi,'diskrit');
...
function buatSoalUL() 
{
	...
	//Kuartil: Soal 1
	randomData = randomSoalDataKelompokDiskrit(20,6,100);
	frekuensi = randomSoalDataTunggalDiskrit(0,6, 10);
	arrData = randomData.split(',');
	arrFrekuensi = frekuensi.split(',');
	shuffle(arrFrekuensi);
	hasilModus = modusKelompok(arrData, arrFrekuensi,'diskrit');
	...
	"\nJumlah Pegawai: "+arrFrekuensi;
	// html = htmlMean(arrData, arrFrekuensi, "kelompok"); 
	arrSoalKuartil[0] = new objSoal(soal,arrOpsi,parseFloat(hasilModus),html);
...
$$(document).on('page:init', function (e, page) {
	...
	if (page.name=="pengenalan")
	{
		  			...
		  			else if(cekJenisData == 'berkelompok')
		  			{
		  				...
		  				else
		  				{
						    ...
						    var nilaiBawah = Math.random() * (+max - +min) + +min;
	  						// nilaiBawah = nilaiBawah.toFixed(3);

						    var min2 = nilaiBawah;
						    var max2 = nilaiBawah + 10;
						    var nilaiAtas = Math.random() * (+max2 - +min2) + +min2;
						    // nilaiAtas = nilaiAtas.toFixed(3);
						    var selisih = nilaiAtas.toFixed(3) - nilaiBawah.toFixed(3);

						    // alert(nilaiBawah.toFixed(3)+'-'+nilaiAtas.toFixed(3)+','+selisih);
  			...
  			if(cekKosong == 0) {
				...
				// varianKFPembilang = 0;
	  			...
	  			if(cekFrekuensi == 'belum' && cekJenisVariabel == 'kontinu' && cekJenisData == 'berkelompok' && cekSifat == 'kuantitatif')
	  			{
	  				...
	  				html += '<li><b>Langkah 2:</b><br> Nilai range nilai dari data yang ada dihitung dengan cara nilai tertinggi dikurangi nilai terendah. Range data tersebut adalah <b>'+nilaiMax+' – '+nilaiMin+' = '+range.toFixed(2)+'</b></li>';

	  				kelasInterval = 1+(3.3*Math.log10(nilaiData.length));
	  				html += '<li><b>Langkah 3:</b><br> Jumlah kelompok atau kelas interval data ditentukan dengan rumus 1 + (3.3 log n), di mana n adalah jumlah data. Jika hasil desimal, maka jumlah kelompok dibulatkan ke atas menjadi bilangan bulat. Jumlah kelompok data tersebut adalah <b>1 + (3.3 log '+nilaiData.length+') = '+kelasInterval.toFixed(2)+' ≈ '+Math.ceil(kelasInterval)+'</b></li>';

	  				lebarKelompok = range.toFixed(2)/Math.ceil(kelasInterval);
	  				html += '<li><b>Langkah 4:</b><br> Lebar tiap kelompok ditentukan dengan cara range dibagi jumlah kelompok. Jika hasil berupa desimal dengan digit lebih dari 2, maka desimal tersebut diambil 2 digit dengan cara dibulatkan ke atas. Berdasarkan perhitungan, lebar kelompok data tersebut adalah <b>'+range.toFixed(2) +' dibagi '+Math.ceil(kelasInterval)+' = '+lebarKelompok.toFixed(2)+'</b></li>';

	  				hitung = lebarKelompok.toFixed(2)*Math.ceil(kelasInterval);
	  				sisa = (hitung - parseFloat(range.toFixed(2)))/2;
	  				html += '<li><b>Langkah 5:</b><br> Sisa data tersebut dihitung menggunakan rumus:<br><img src=\"img/persamaan21.png\"><br>Sisa data tersebut adalah <b>'+sisa.toFixed(2)+'</b>, di mana lebar dikali jumlah kelompok adalah '+hitung+'. Lalu '+hitung+' dikurangi range '+range.toFixed(2)+', kemudian dibagi 2</li>';

	  				batasBawah = nilaiMin - sisa.toFixed(2);
	  				batasAtas = batasBawah + Math.ceil(kelasInterval);
	  				html += '<li><b>Langkah 6:</b><br> Kelas dibuat dengan menentukan batas bawah dan batas atas setiap kelompok. Batas bawah kelompok didapatkan dari nilai minimum dikurang sisa, sedangkan batas atas kelompok didapatkan dari batas bawah kelompok ditambah jumlah kelompok. Batas bawah kelompok pertama data tersebut adalah <b>'+nilaiMin+' – '+sisa.toFixed(2)+' = '+batasBawah.toFixed(2)+'</b>, sedangkan batas atas pertama kelompok data tersebut adalah <b>'+batasBawah.toFixed(2)+' + '+Math.ceil(kelasInterval)+' = '+batasAtas.toFixed(2)+'</b></li>';
					...
					for(var i=1; i<=Math.ceil(kelasInterval); i++)
					{
						frekuensi = 0;
						htmlTable += '<tr>';
						htmlTable += '<td><input type=\"text\" value=\"'+((batasBawah).toFixed(3))+'-'+(batasBawah+lebarKelompok).toFixed(3)+'\"></td>';
						...
						titikTengah = ((batasBawah+batasBawah+lebarKelompok).toFixed(2))/2;
	  			...
	  			else if(cekFrekuensi == 'belum' && cekJenisVariabel == 'diskrit' && cekJenisData == 'berkelompok' && cekSifat == 'kuantitatif')
	  			{
	  				...
	  				kelasInterval = 1+3.3*Math.log10(nilaiData.length);
	  				html += '<li><b>Langkah 3:</b><br> Jumlah kelompok atau kelas interval data ditentukan dengan rumus 1 + (3.3 log n), di mana n adalah jumlah data. Jumlah kelompok data tersebut adalah 1 + 3.3 log '+nilaiData.length+') = '+kelasInterval.toFixed(2)+' ≈ '+Math.ceil(kelasInterval)+'</b></li>';
		...
		$$("#tutupPengerjaanHistogram").on('click', function() {
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" class="histoRVT" id="histoRowVariabelTunggal'+histoCountRow+'" name="" value="" placeholder="Isi Variabel">';
  		...
  		{ 
  			...
  			$$("#histoChart").show();
   			//langkah-langkah
  			var html = "";
  			html = '<li><b>Langkah 1:</b><br> Garis horizontal dibuat dengan berisi nilai titik tengah kelas variabel yang diamati</li>';
		...
		$$("#tutupPengerjaanPoligon").on('click', function() {
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" id="poligonRowVariabelTunggal'+poligonCountRow+'" name="" value="" placeholder="Isi Variabel">';
			cell2.innerHTML = '<input type="number" step="1" min="0" id="poligonRowFrekuensiTunggal'+poligonCountRow+'" name="" value="" placeholder="Isi Frekuensi">';
  		...
  		{ 
			...
			html += '<li><b>Langkah 4:</b><br> Titik dibuat setinggi frekuensi tiap kelas, lalu tarik garis antar titik yang dibuat</li>';
  			...
  			$('.poligonRVT').each(function(i, obj) {
				...
				// if ($(this).val()=="" || obj.variable==" ") //jika ada var yg sama
			 //  	{
			 //  		found=true;
			 //  		msg=msg+"Variable tidak boleh kosong\n";
			 //  	}
		...
		$$("#tutupPengerjaanScatter").on('click', function() {
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" id="scatterRowVariabelTunggal'+scatterCountRow+'" name="" value="" placeholder="Isi Variabel">';
			cell2.innerHTML = '<input type="number" step="1" min="0" id="scatterRowFrekuensiTunggal'+scatterCountRow+'" name="" value="" placeholder="Isi Frekuensi">';
		...
		$$("#tutupPengerjaanLingkaran").on('click', function() {
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" id="lingkaranRowVariabelTunggal'+lingkaranCountRow+'" name="" value="" placeholder="Isi Variabel">';
			cell2.innerHTML = '<input type="number" step="1" min="0" id="lingkaranRowFrekuensiTunggal'+lingkaranCountRow+'" name="" value="" placeholder="Isi Frekuensi">';
		});
		...
		var sbsCountRow = 1;
  		...
  		$$('#sbAutoInput').click(function()
  		{
			...
			{
				$$('#sbsRowVariabelTunggal'+i).val(acakKata[i-1]);

				//update frekuensi
			    var minFreq = 1;
			    var maxFreq = 10;
			    var randomFreq = Math.floor(Math.random() * (+maxFreq - +minFreq)) + +minFreq;
			    $$('#sbsRowFrekuensiTunggal'+i).val(randomFreq);
			}
  		});
	...
	else if (page.name=="soal")
	{
		...
		//----- Pilih Jenis Inputan Distribusi Frekuensi -----//
  		...
  		$$('#odTambahRowTunggal').click(function()
  		{
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" class="grafikRVT" id="odRowVariabelTunggal'+countRow+'" name="" value="" placeholder="Isi Variabel">';
			cell2.innerHTML = '<input type="text" class="grafikRFT" step="1" min="0" id="odRowFrekuensiTunggal'+countRow+'" name="" value="" placeholder="Isi Frekuensi">';
  		});
  		//----- Hapus Baris Data Tunggal -----//
  		...
  		$$('#odTambahRowKelompok').click(function()
  		{
			...
			var cell1 = row.insertCell(0);
			var cell2 = row.insertCell(1);
			cell1.innerHTML = '<input type="text" class="grafikRVT" id="odRowVariabelKelompok'+countRowKelompok+'" name="" value="" placeholder="Isi Variabel">';
			cell2.innerHTML = '<input type="number" class="grafikRFT" step="1" min="0" id="odRowFrekuensiKelompok'+countRowKelompok+'" name="" value="" placeholder="Isi Frekuensi">';
  		});
  		...
  		$$('#odKerjakan').click(function()
  		{
			...
			if(cekFrekuensi == "belum") {
  			...
  			// PENGERJAAN UKURAN PEMUSATAN DATA
			if(cekKosong == 0 && cekpodUKP == "odUKP") {
  			...
  			if (cekKosong == 0 && cekpodGrafik == "odGrafik") {
  				if(cekSifat == 'kuantitatif') {
				    ...
				    if (msg=="")
				    {
						$$("#odHistoLangkahPengerjaan").html(html);
